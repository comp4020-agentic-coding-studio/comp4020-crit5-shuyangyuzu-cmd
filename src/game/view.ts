...
import { COLLECTOR_IDS, type AcquiredLot, type CollectorId } from "./types";

export interface PublicCollectorView {
  id: CollectorId;
  name: string;
  holdings: Partial<Record<ArtistId, number>>;
  // Exact-identity purchase records are collection composition, exactly like
  // holdings — public for every collector at every phase. Never gated behind
  // `reveal`, which only ever governs cash/collectionValue/netWorth below.
...
export function buildPublicView(state: GameState): PublicView {
  ...
  const collectors = COLLECTOR_IDS.map((id): PublicCollectorView => {
    ...
    if (!reveal) {
      return { id, name: collector.name, holdings: collector.holdings, acquiredLots: collector.acquiredLots };
    }
    ...
    return {
      ...
      acquiredLots: collector.acquiredLots,
      cash: collector.cash,
      collectionValue,
